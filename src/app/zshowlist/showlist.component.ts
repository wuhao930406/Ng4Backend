...
export class ShowlistComponent implements OnInit {
  ...
  getDatas(){
    ...
    let urls = AllData.base_url+PortName.exhibition_list;
    if(this.single=='0'){
      this.action='批量发布';
      this.state = '1';
  ...
  setAction(){
    let url = AllData.base_url+PortName.exhibition_publishs;
  ...
  deleteAll(key){
    let url = AllData.base_url+PortName.exhibition_deletes;
    let id = sessionStorage.getItem(SessionKey.ID),data;
    if(key){
