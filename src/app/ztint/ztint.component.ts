...
@Component({
  selector: 'ztint-list',
  templateUrl: './ztint.component.html',
  styleUrls: [ './ztint.component.css' ]
})

export class ZtintComponent implements OnInit {
  ...
  getDatas(){
    ...
    let urls = AllData.base_url+PortName.course_list;
    if(this.single=='0'){
      this.action='批量发布';
      this.state = '1';
  ...
  setAction(){
    let url = AllData.base_url+PortName.course_publishs;
  ...
  deleteAll(key){
    let url = AllData.base_url+PortName.course_deletes;
    let id = sessionStorage.getItem(SessionKey.ID),data;
    if(key){
