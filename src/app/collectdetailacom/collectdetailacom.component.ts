...
@Component({
  selector: 'collectdetailacom-com',
  templateUrl: "./collectdetailacom.component.html",
  styleUrls: [ './collectdetailacom.component.css' ]
})
export class CollectDetailacnComponent implements OnInit {
  ids:string;
  dataList = {};
  isVisible = false;
  examine:string = '';
  ...
  changeState(key){
    ...
    let url = AllData.base_url+PortName.lcollect_examine;
  ...
  getDatas(){
    let id = sessionStorage.getItem(SessionKey.ID);
    let url = AllData.base_url+PortName.lcollect_get;
